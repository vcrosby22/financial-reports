...
<div class="subtitle">{now} &nbsp;|&nbsp; Data: Technical + {"Macro (FRED) + " if macro_data and macro_data.indicators else ""}{"Fundamentals" if fundamentals else "Technical only"} &nbsp;|&nbsp; Auto-updated weekdays before market open</div>
...
<strong>Data sources:</strong> yfinance (unofficial){", FRED API (official)" if macro_data else ""}.
This is AI-generated analysis for educational purposes, not financial advice.<br>
...
def _section_macro(macro_data: MacroSnapshot) -> str:
    ...
    for ind in macro_data.indicators:
        signal_class = {"critical": "tag-critical", "warning": "tag-warning", "bearish": "tag-warning",
                        "bullish": "tag-strong", "neutral": "tag-info"}.get(ind.signal, "tag-info")
...
def _collapsible(title: str, content: str, open_default: bool = False) -> str:
    open_attr = " open" if open_default else ""
    return f"""
<details class="section-collapse"{open_attr}>
