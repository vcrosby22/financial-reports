...
/* Mobile-first KPI hierarchy: 3 primary cards above the fold, 3 secondary
   collapsed behind a toggle. On wider screens all 6 show together. */
.kpi-row .kpi-primary {{ display: block; }}
.kpi-row .kpi-secondary {{ display: none; }}
...
.kpi-row.kpi-secondary-shown .kpi-secondary {{ display: block; }}
...
(function () {{
  // KPI "More indices" toggle — mobile-only (CSS hides the button at >=600px).
  var btn = document.querySelector("[data-kpi-toggle]");
  if (!btn) return;
  var row = document.getElementById("kpi-row");
  if (!row) return;
  btn.addEventListener("click", function () {{
    var shown = row.classList.toggle("kpi-secondary-shown");
    btn.setAttribute("aria-expanded", shown ? "true" : "false");
    var n = row.querySelectorAll(".kpi-secondary").length;
    btn.innerHTML = shown
      ? "Hide extra indices &#9652;"
      : "More indices (" + n + ") &#9662;";
  }});
...
def _section_kpi_cards(
    ...
    if secondary:
        toggle_html = (
            ...
            'data-kpi-toggle '
            f'aria-expanded="false">More indices ({len(secondary)}) &#9662;</button>'
        )
