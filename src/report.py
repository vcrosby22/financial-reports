...
def _build_html(
    ...
    panels.append('<div class="tab-panel" data-tab="bonds" hidden inert>')
    panels.append(bonds_section)
    panels.append('</div>')

    panels.append('<div class="tab-panel" data-tab="cascade" hidden inert>')
    panels.append(cascade_section)
    panels.append('</div>')

    panels.append('<div class="tab-panel" data-tab="markets" hidden inert>')
    panels.append(market_section)
    if fundamentals_section:
        panels.append(fundamentals_section)
    panels.append('</div>')

    panels.append('<div class="tab-panel" data-tab="macro" hidden inert>')
    panels.extend(macro_parts)
    panels.append('</div>')

    panels.append('<div class="tab-panel" data-tab="opportunities" hidden inert>')
    ...
    panels.append('<div class="tab-panel" data-tab="signals" hidden inert>')
    panels.extend(signals_parts)
    panels.append('</div>')

    panels.append('<div class="tab-panel" data-tab="crisis" hidden inert>')
    panels.extend(crisis_parts)
    panels.append('</div>')

    if extra_parts:
        panels.append('<div class="tab-panel" data-tab="reference" hidden inert>')
...
/* Freshness pill — colors snapshot age so reader sees stale builds at a glance.
   Set client-side from data-snapshot-utc on body. */
.freshness-pill {{
...
.freshness-pill.fresh {{ color: #22c55e; border-color: #16a34a44; }}
.freshness-pill.stale {{ color: #eab308; border-color: #ca8a0444; background: rgba(234, 179, 8, 0.08); }}
.freshness-pill.very-stale {{ color: #ef4444; border-color: #dc262644; background: rgba(239, 68, 68, 0.1); }}
.table-scroll {{
...
/* Mobile KPI hierarchy: all 6 indices show by default; toggle hides the
   secondary indices (Dow / Nasdaq / Brent etc.) for readers who want to
   focus on Risk + S&P + VIX only. On wider screens all 6 show together
   and the toggle is hidden. */
.kpi-row .kpi-primary {{ display: block; }}
.kpi-row .kpi-secondary {{ display: block; }}
.kpi-secondary-toggle {{
  display: block;
  margin: 0 0 1.25rem;
...
.kpi-secondary-toggle:hover {{
  color: var(--text); border-color: var(--text-dim);
}}
.kpi-row.kpi-secondary-hidden .kpi-secondary {{ display: none; }}

/* Macro neutral-row collapse — readers care about non-neutral signals;
   stable indicators are noise that crowds the screen. Hidden by default;
   user can reveal with the table-scoped toggle. */
.macro-table.is-neutral-hidden tr.macro-row-neutral {{ display: none; }}
.macro-neutral-toggle {{
...
.macro-neutral-toggle:hover {{
  color: var(--text); border-color: var(--text-dim);
}}

/* Opportunities — filter chips + top-5 collapse.
   Default state: rows with data-extra="true" are hidden until the
   "Show more" button is clicked, AND any row failing the active
   filter chips also gets .opp-row-filtered which display:none. */
.opp-filter-bar {{
  display: flex; flex-wrap: wrap; align-items: center;
  gap: 0.5rem 1rem;
...
.opp-chip.is-active {{
  background: var(--text-dim); color: var(--bg, #0f172a);
  border-color: var(--text-dim);
}}
.opp-row[data-extra="true"] {{ display: none; }}
.opp-group.is-fully-shown .opp-row[data-extra="true"] {{ display: block; }}
.opp-row.opp-row-filtered {{ display: none !important; }}
.opp-show-all {{
  display: block;
  margin: 0 0 1.25rem;
  padding: 0.4rem 0.75rem;
...
@media (min-width: 600px) {{
  ...
  /* Phone-landscape gets the wider layout: hidden columns reveal,
     rotate hint disappears (since the user already followed it). */
  .col-m-hide {{ display: table-cell !important; }}
  .mobile-rotate-hint {{ display: none; }}
  .subtitle-detail {{ display: block; }}
  /* All KPI cards visible at landscape/tablet+; mobile toggle hidden.
     !important overrides .kpi-row.kpi-secondary-hidden when a mobile
     user toggled "Hide" then rotated. */
  .kpi-row .kpi-secondary,
  .kpi-row.kpi-secondary-hidden .kpi-secondary {{ display: block !important; }}
  .kpi-secondary-toggle {{ display: none; }}
}}

/* Belt-and-suspenders: any landscape orientation also unhides columns
   even if width is below 600px (very small phones in landscape). */
...
<body data-snapshot-utc="{now_et.astimezone(timezone.utc).isoformat()}">
...
{fred_block}
<div class="subtitle-line">Snapshot generated: <strong>{escape(now)}</strong> <span id="freshness-pill" class="freshness-pill" aria-live="polite">checking…</span> <span class="subtitle-detail" style="color:var(--text-dim);">(HTML build / CI time — not the same as market close).</span></div>
...
(function () {{
  // Freshness pill — color-codes how stale the snapshot is.
  // Green: < 6h, Amber: 6-24h, Red: > 24h.
  var pill = document.getElementById("freshness-pill");
  if (!pill) return;
  var snapIso = document.body.getAttribute("data-snapshot-utc");
  if (!snapIso) {{ pill.style.display = "none"; return; }}
  var snap = new Date(snapIso);
  var now = new Date();
  var hours = (now - snap) / 36e5;
  var label, cls;
  if (hours < 1) {{
    label = "just now";
    cls = "fresh";
    ...
    label = Math.round(hours / 24) + " days old";
    cls = "very-stale";
  }}
  pill.className = "freshness-pill " + cls;
  pill.textContent = label;
  pill.title = "Snapshot generated " + snap.toLocaleString();
}})();
(function () {{
  // KPI "Show / Hide indices" toggle — mobile-only (CSS hides the button at >=600px).
  // Default state: all indices visible (Dow / Nasdaq / Brent etc.).
  // Click hides the secondary indices; click again restores.
  var btn = document.querySelector("[data-kpi-toggle]");
  if (!btn) return;
  var row = document.getElementById("kpi-row");
  if (!row) return;
  btn.addEventListener("click", function () {{
    var hidden = row.classList.toggle("kpi-secondary-hidden");
...
(function () {{
  // Opportunities filter chips + "Show all" expansion.
  // State per facet (horizon / risk-band / confidence) — single-select per
  // facet with "all" as default. Filtered rows get .opp-row-filtered.
  // Independent per direction "Show all" expands data-extra rows.
  var filterBar = document.querySelector(".opp-filter-bar");
  if (!filterBar) return;

  var state = {{ horizon: "all", "risk-band": "all", confidence: "all" }};
  var rows = document.querySelectorAll(".opp-row");
  var groups = document.querySelectorAll(".opp-group");

  function rowMatches(row) {{
    for (var facet in state) {{
      var v = state[facet];
      if (v === "all") continue;
      if (row.getAttribute("data-" + facet) !== v) return false;
    }}
    return true;
  }}
  ...
  function applyFilters() {{
    for (var i = 0; i < rows.length; i++) {{
      var r = rows[i];
      if (rowMatches(r)) {{
        r.classList.remove("opp-row-filtered");
      }} else {{
        r.classList.add("opp-row-filtered");
      }}
    }}
    // Per-group "no matches" state and Show-all button visibility.
    for (var j = 0; j < groups.length; j++) {{
      var g = groups[j];
      var direction = g.getAttribute("data-opp-group");
      var groupRows = g.querySelectorAll(".opp-row");
      var visible = 0, hiddenByExtra = 0;
      for (var k = 0; k < groupRows.length; k++) {{
        var gr = groupRows[k];
        if (gr.classList.contains("opp-row-filtered")) continue;
        if (gr.getAttribute("data-extra") === "true" && !g.classList.contains("is-fully-shown")) {{
          hiddenByExtra++;
        }} else {{
          visible++;
        }}
      }}
      // Toggle a sibling "no matches" message.
      var emptyId = "opp-empty-" + direction;
      var empty = document.getElementById(emptyId);
      if (visible === 0 && hiddenByExtra === 0) {{
        if (!empty) {{
          empty = document.createElement("div");
          empty.id = emptyId;
          empty.className = "opp-empty-message";
          empty.textContent = "No " + direction + " opportunities match these filters.";
          g.parentNode.insertBefore(empty, g.nextSibling);
        }}
      }} else if (empty) {{
        empty.parentNode.removeChild(empty);
      }}
      // Show-all button: hide if no rows hidden by extra-collapse.
      var btn = document.querySelector('.opp-show-all[data-opp-direction="' + direction + '"]');
      if (btn) {{
        btn.style.display = (hiddenByExtra > 0 && !g.classList.contains("is-fully-shown")) ? "block" : "none";
        if (hiddenByExtra > 0) {{
          btn.innerHTML = "Show " + hiddenByExtra + " more &#9662;";
        }}
      }}
    }}
  }}

  var chips = filterBar.querySelectorAll(".opp-chip");
  for (var i = 0; i < chips.length; i++) {{
    chips[i].addEventListener("click", function (ev) {{
      var spec = ev.currentTarget.getAttribute("data-opp-filter") || "";
      var parts = spec.split(":");
      if (parts.length !== 2) return;
      var facet = parts[0], value = parts[1];
      state[facet] = value;
      // Mark the active chip in this facet.
      var groupChips = filterBar.querySelectorAll('[data-opp-filter^="' + facet + ':"]');
      for (var k = 0; k < groupChips.length; k++) {{
        groupChips[k].classList.remove("is-active");
      }}
  ...
  var showAlls = document.querySelectorAll(".opp-show-all");
  for (var s = 0; s < showAlls.length; s++) {{
    showAlls[s].addEventListener("click", function (ev) {{
      var direction = ev.currentTarget.getAttribute("data-opp-direction");
      var g = document.querySelector('.opp-group[data-opp-group="' + direction + '"]');
      if (!g) return;
      g.classList.add("is-fully-shown");
...
(function () {{
  // Macro "stable indicators" toggle — pairs each button with its table by
  // the data-macro-toggle / data-macro-table id. Hides neutral rows by default.
  var buttons = document.querySelectorAll("[data-macro-toggle]");
  for (var i = 0; i < buttons.length; i++) {{
    (function (btn) {{
      var id = btn.getAttribute("data-macro-toggle");
      var table = document.querySelector('[data-macro-table="' + id + '"]');
      if (!table) return;
      var n = table.querySelectorAll("tr.macro-row-neutral").length;
      var nounMatch = btn.textContent.match(/stable (\\w+)/);
      var noun = nounMatch ? nounMatch[1] : "indicators";
      btn.addEventListener("click", function () {{
        var shown = !table.classList.contains("is-neutral-hidden");
        if (shown) {{
          table.classList.add("is-neutral-hidden");
          btn.setAttribute("aria-expanded", "false");
          btn.innerHTML = "Show " + n + " stable " + noun + " &#9662;";
        }} else {{
          table.classList.remove("is-neutral-hidden");
          btn.setAttribute("aria-expanded", "true");
          btn.innerHTML = "Hide " + n + " stable " + noun + " &#9652;";
        }}
      }});
    }})(buttons[i]);
  }}
...
(function () {{
  ...
  function openDefaultSection(tabName) {{
    if (tabName !== "cascade") {{
      return;
    }}
    var panel = document.querySelector('.tab-panel[data-tab="cascade"]');
    var section = panel ? panel.querySelector("#supply-chain") : null;
  ...
  function activateTab(tabName, scroll) {{
    ...
    for (var j = 0; j < panels.length; j++) {{
      var isActive = panels[j].getAttribute("data-tab") === tabName;
      if (isActive) {{
  ...
  if (initHash && initHash.length > 1) {{
    activateTab(decodeURIComponent(initHash.slice(1)), true);
  }}
}})();
</script>
</body>
...
def _section_kpi_cards(
    ...
    Mobile hierarchy (Storytelling-with-Data: limit cognitive load above the fold):
      Primary    — Risk Score, S&P 500 (with drawdown), VIX
      Secondary  — Dow, NASDAQ, Brent Crude  (collapsed behind toggle on phone)

    On screens ≥ 600px (landscape / tablet / desktop) the toggle vanishes
    and all six cards show together.
    """
    uncapped = _health_uncapped_score(health)
    score_display = str(uncapped) if uncapped > 100 else str(health.score)

    def _kpi(label: str, value: str, color: str, sub: str = "", tier: str = "primary") -> str:
        return (
            f'<div class="kpi-{tier}" style="background:var(--surface);border-radius:0.75rem;'
    ...
    def _index_card(label: str, data: dict | None, tier: str = "secondary") -> str | None:
        if not data or not data.get("price"):
            return None
        chg = data.get("change_pct_1d", 0) or 0
        arrow = "&#9650;" if chg >= 0 else "&#9660;"
        chg_color = "var(--green)" if chg >= 0 else "var(--red)"
        return _kpi(label, f"{data['price']:,.0f}", chg_color, f"{arrow} {chg:+.2f}%", tier=tier)

    def _sp500_primary_card(data: dict | None) -> str | None:
        """S&P card with drawdown-from-52w-high subtitle for primary slot."""
        if not data or not data.get("price"):
            return None
        chg = data.get("change_pct_1d", 0) or 0
        arrow = "&#9650;" if chg >= 0 else "&#9660;"
        chg_color = "var(--green)" if chg >= 0 else "var(--red)"
        sub_lines = [f"{arrow} {chg:+.2f}% today"]
        high_52w = data.get("fifty_two_week_high")
        if high_52w and data["price"]:
            dd_pct = (data["price"] - high_52w) / high_52w * 100
            # Drawdown framing: red ≤ -10% (correction territory),
            # dim text -10% to -1%, green when within 1% of the high.
            if dd_pct >= -1:
                sub_lines.append(
                    '<span style="color:var(--green);">at 52-week high</span>'
                )
            else:
                dd_color = "var(--red)" if dd_pct <= -10 else "var(--text-dim)"
                sub_lines.append(
                    f'<span style="color:{dd_color};">{dd_pct:.1f}% from 52-week high</span>'
                )
        sub_html = "<br>".join(sub_lines)
        return _kpi("S&amp;P 500", f"{data['price']:,.0f}", chg_color, sub_html, tier="primary")
    ...
    cards = [_kpi("Risk Score", f"{score_display}{delta_str}", risk_color, level_subtitle, tier="primary")]

    sp_card = _sp500_primary_card(sp500)
    if sp_card:
    ...
    if oil and oil.get("price"):
        ...
        secondary.append(_kpi("Brent Crude", f"${oil['price']:.2f}", oil_color, "per barrel", tier="secondary"))

    cards.extend(secondary)

    total = len(cards)
    kpi_class = "kpi-row kpi-6" if total >= 5 else "kpi-row"

    toggle_html = ""
    if secondary:
        toggle_html = (
            '<button type="button" class="kpi-secondary-toggle" '
            'data-kpi-toggle '
            f'aria-expanded="true">Hide indices &#9652;</button>'
        )

    return (
        f'<div class="{kpi_class}" id="kpi-row">'
        + "".join(cards)
        + '</div>'
        + toggle_html
    )
...
<tbody>"""
    ...
    for category, label in categories:
        ...
        # Indices opens by default so the reader sees the headline market
        # data (S&P, Dow, Nasdaq, VIX, Brent) the moment they hit Markets.
        # Other categories stay collapsed to keep the tab scannable.
        category_open = category == "indices"
        ...
        if category in ("stocks", "etfs") and len(items) > 12:
            ...
            parts.append(_collapsible(f"{label} ({len(items)})", content, open_default=category_open))
        else:
...
def _section_macro(macro_data: MacroSnapshot) -> str:
    ...
    def _macro_indicator_row(ind) -> str:
        ...
        change = f"{ind.change:+,.2f}" if ind.change is not None else "—"
        row_class = " class='macro-row-neutral'" if ind.signal == "neutral" else ""
        return (
            f"<tr{row_class}><td><strong>{escape(ind.name)}</strong>"
    ...
    neutral_count = sum(1 for ind in macro_data.indicators if ind.signal == "neutral")
    neutral_toggle = ""
    if neutral_count:
        neutral_toggle = (
            f'<button type="button" class="macro-neutral-toggle" '
            f'data-macro-toggle="macro-main" aria-expanded="false">'
            f'Show {neutral_count} stable indicators &#9662;</button>'
        )
...
def _section_inflation(macro_data: MacroSnapshot) -> str:
    ...
    for ind in components:
        ...
        is_neutral = ind.signal == "neutral"
        if is_neutral:
            comp_neutral_count += 1
        row_class = " class='macro-row-neutral'" if is_neutral else ""
        comp_rows.append(
            f"<tr{row_class}><td><strong>{escape(ind.name)}</strong></td>"
            f"<td style='text-align:right;white-space:nowrap;'>{ind.value:,.2f}</td>"
    ...
    if comp_rows:
        comp_toggle = ""
        if comp_neutral_count:
            comp_toggle = (
                f'<button type="button" class="macro-neutral-toggle" '
                f'data-macro-toggle="macro-comp" aria-expanded="false">'
                f'Show {comp_neutral_count} stable components &#9662;</button>'
            )
        comp_html = (
            '<div class="card table-scroll wide-min sticky-first-col table-edge-hint">'
            '<table class="macro-table is-neutral-hidden" data-macro-table="macro-comp"><thead><tr>'
    ...
    if exp_rows:
        exp_toggle = ""
        if exp_neutral_count:
            exp_toggle = (
                f'<button type="button" class="macro-neutral-toggle" '
                f'data-macro-toggle="macro-exp" aria-expanded="false">'
                f'Show {exp_neutral_count} stable measures &#9662;</button>'
            )
        exp_html = (
            ...
            '<div class="card table-scroll wide-min sticky-first-col table-edge-hint">'
            '<table class="macro-table is-neutral-hidden" data-macro-table="macro-exp"><thead><tr>'
            '<th>Measure</th><th style="text-align:right;width:5rem;">Value</th>'
            '<th style="width:4.5rem;">Signal</th><th class="col-m-hide">Assessment</th>'
            f'</tr></thead><tbody>{"".join(exp_rows)}</tbody></table></div>'
            f'{exp_toggle}</div>'
        )
...
def _section_opportunities(opportunities: list[Opportunity], health: MarketHealthReport) -> str:
    ...
    def _risk_band(score: int) -> str:
        """Bucket risk score into low/medium/high for filter chips."""
        if score <= 3:
            return "low"
        if score <= 6:
            return "medium"
    ...
    def _horizon_band(label: str) -> str:
        """Normalize horizon labels to short/medium/long for filter chips."""
        s = (label or "").lower()
        if "short" in s:
            return "short"
    ...
    def _opp_row(opp: Opportunity, rank: int) -> str:
        ...
        is_extra = "true" if rank >= 5 else "false"
        return (
            f'<details class="opp-row" '
            f'data-direction="{opp.direction}" '
            f'data-horizon="{_horizon_band(opp.horizon_label)}" '
            f'data-risk-band="{_risk_band(opp.risk_score)}" '
            f'data-confidence="{opp.confidence}" '
            f'data-extra="{is_extra}" '
            f'style="border:1px solid var(--border);border-radius:0.5rem;margin-bottom:0.5rem;overflow:clip;">'
    ...
    def _build_direction_block(group: list, direction: str) -> str:
        if not group:
            empty = "long" if direction == "long" else "short"
            return f'<div class="card" style="color:var(--text-dim);">No {empty} opportunities identified in current conditions.</div>'
        rows = "".join(_opp_row(o, rank=i) for i, o in enumerate(group))
        n_extra = max(0, len(group) - 5)
        toggle = ""
        if n_extra:
            toggle = (
                f'<button type="button" class="opp-show-all" '
                f'data-opp-direction="{direction}" aria-expanded="false">'
                f'Show {n_extra} more &#9662;</button>'
            )
        return (
            f'<div class="opp-group" data-opp-group="{direction}">'
            f'{rows}'
            f'</div>{toggle}'
        )

    long_cards = _build_direction_block(longs, "long")
    short_cards = _build_direction_block(shorts, "short")

    # Filter chip bar — pills toggle which categories show. State combines with
    # the top-5 collapse via JS in the page-level script block.
    filter_bar = (
        '<div class="opp-filter-bar" role="toolbar" aria-label="Opportunity filters">'
        '<span class="opp-filter-label">Filter:</span>'
        '<div class="opp-filter-group" data-opp-facet="horizon">'
        '<span class="opp-facet-name">Horizon</span>'
        '<button type="button" class="opp-chip is-active" data-opp-filter="horizon:all">All</button>'
        '<button type="button" class="opp-chip" data-opp-filter="horizon:short">Short</button>'
        '<button type="button" class="opp-chip" data-opp-filter="horizon:medium">Medium</button>'
        '<button type="button" class="opp-chip" data-opp-filter="horizon:long">Long</button>'
        '</div>'
        '<div class="opp-filter-group" data-opp-facet="risk-band">'
        '<span class="opp-facet-name">Risk</span>'
        '<button type="button" class="opp-chip is-active" data-opp-filter="risk-band:all">All</button>'
        '<button type="button" class="opp-chip" data-opp-filter="risk-band:low">Low</button>'
        '<button type="button" class="opp-chip" data-opp-filter="risk-band:medium">Med</button>'
        '<button type="button" class="opp-chip" data-opp-filter="risk-band:high">High</button>'
        '</div>'
        '<div class="opp-filter-group" data-opp-facet="confidence">'
        '<span class="opp-facet-name">Confidence</span>'
        '<button type="button" class="opp-chip is-active" data-opp-filter="confidence:all">All</button>'
        '<button type="button" class="opp-chip" data-opp-filter="confidence:high">High</button>'
        '<button type="button" class="opp-chip" data-opp-filter="confidence:medium">Med</button>'
        '<button type="button" class="opp-chip" data-opp-filter="confidence:low">Low</button>'
        '</div>'
        '</div>'
    )
